...
export default function LanguageSwitch({ withDisplay = false }) {
	...
	const [currentLanguage, setCurrentLanguage] = useState(lang === 'en' ? 'English' : 'Deutsch');
	...
	const changeLanguage = (lang: string) => {
		push({ pathname, query }, { pathname, query }, { locale: lang === 'Deutsch' ? 'de' : 'en' });
	};

	return (
		<Dropdown
			options={allLanguages.filter((language) => language.name !== currentLanguage)}
